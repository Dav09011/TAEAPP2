...
class RevenueCatPaywallScreen extends StatefulWidget {
  ...
  @override
  State<RevenueCatPaywallScreen> createState() =>
...
class _RevenueCatPaywallScreenState extends State<RevenueCatPaywallScreen>
  ...
  Future<void> _showPaywall() async {
    ...
    try {
      ...
      if (offering == null) {
        _snack('Sin offering configurado');
        return;
      }
  ...
  Future<void> _restore() async {
    ...
    final bool wasActive =
        _customerInfo != null ? _isActive(_customerInfo!) : false;
    try {
      await Purchases.invalidateCustomerInfoCache();
      ...
      builder:
          (_) => _SuccessDialog(
            ...
            expiry: _formatExpiry(
              info.entitlements.all[_entitlementId]?.expirationDate,
            ),
          ),
    );
  }
  ...
  void _snack(String msg) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          msg,
          style: const TextStyle(color: _P.bg, fontWeight: FontWeight.w600),
        ),
...
class _DiagPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint =
        Paint()
          ..color = const Color(0xFFD90429).withValues(alpha: 0.03)
          ..strokeWidth = 32
          ..style = PaintingStyle.stroke;

    final offset = t * 64;
    for (
      double x = -size.height + offset % 64;
    ...
    final triPaint =
        Paint()
          ..color = const Color(0xFFD90429).withValues(alpha: 0.055)
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: Icon(
        icon,
        size: 16,
        color: onTap == null ? _P.inkLight : _P.inkMid,
      ),
    ),
  );
}
  ...
  Widget build(BuildContext context) => const Center(
    child: Column(
      ...
      children: [
        ...
        Text(
          'Cargando…',
          style: TextStyle(color: _P.inkMid, fontSize: 13, letterSpacing: 0.5),
        ),
      ],
    ),
  );
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: _P.red.withValues(alpha: 0.3)),
            ),
            child: const Icon(
          ...
          const Text(
            'Verifica tu API key de RevenueCat.',
            textAlign: TextAlign.center,
...
class _Body extends StatelessWidget {
  ...
  const _Body({
    ...
    required this.loadingPaywall,
    required this.loadingRestore,
          ...
          decoration: BoxDecoration(
            ...
            boxShadow:
                isPremium
                    ? [
                      BoxShadow(
                        color: _P.red.withValues(alpha: 0.28),
                        blurRadius: 22,
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Icon(
                          ...
                          color: isPremium ? Colors.white : _P.inkLight,
                          size: 18,
                        ),
                        const SizedBox(width: 8),
                        Text(
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color:
                                isPremium
                                    ? const Color(0xFF7FFFCA)
                                    : _P.inkLight.withValues(alpha: 0.4),
                            boxShadow:
                                isPremium
                                    ? [
                                      const BoxShadow(
                                        color: Color(0x807FFFCA),
                                        blurRadius: 8,
                                      ),
                                    ]
                                    : [],
                    ...
                    if (isPremium && customerInfo != null) ...[
                      const SizedBox(height: 16),
                      Text(
                        _planLabel(subscriptionType(customerInfo!)),
                        style: const TextStyle(
                          color: Colors.white,
                          fontSize: 30,
                          fontWeight: FontWeight.w900,
  ...
  Widget build(BuildContext context) {
    const benefits = [
      (
        ...
        'Clases en video ilimitadas',
        'Todos los niveles, sin restricciones.',
      ),
      (
        Icons.emoji_events_rounded,
        'Seguimiento de progreso',
        'Registra tus rangos y avances.',
      ),
      (
        Icons.calendar_month_rounded,
        'Agenda de entrenamientos',
        'Planea tu semana dojo.',
      ),
      (
        Icons.support_agent_rounded,
        'Soporte prioritario',
        'Respuesta directa del equipo.',
      ),
    ];
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                desc,
                style: const TextStyle(color: _P.inkLight, fontSize: 11),
              ),
            ],
          ),
        ),
    ...
    return AnimatedBuilder(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow:
                  isDisabled
                      ...
                      : [
                        BoxShadow(
                          color: _P.red.withValues(
                            alpha: 0.22 + pulseCtrl.value * 0.18,
                          ),
                          blurRadius: 20,
      ...
      child: _RedButton(
        label:
            loading
                ? 'Cargando…'
                : (isPremium ? 'Gestionar mi plan' : 'UNIRME AHORA'),
        icon:
            loading
                ? null
                : (isPremium
              ...
              loading
                  ...
                  : Text(
                    ...
                    style: TextStyle(
                      ...
                      color:
                          isDisabled
                              ? _P.inkLight.withValues(alpha: 0.5)
                              : _P.inkMid,
                ...
                child: Column(
                  children: [
                    ...
                    const _BenefitTile(
                      ...
                      desc: 'Acceso completo a TAEAPP',
                    ),
                    const SizedBox(height: 8),
                    const _BenefitTile(
                      icon: Icons.block_rounded,
                      title: 'Sin anuncios',
                      desc: 'Entrena sin interrupciones',
                    ),
                    const SizedBox(height: 8),
                    const _BenefitTile(
                      icon: Icons.support_agent_rounded,
                      title: 'Soporte prioritario',
                      desc: 'Ayuda directa del equipo',
                    ),
                    const SizedBox(height: 24),
                    _RedButton(
